...
bool isWin(string[] board)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        if (board.Skip(i*5).Take(5).Count(x => x.Length == 0) == 5)
            return true;
    }
    ...
    for (int x = 0; x < 5; ++x)
    {
        if (board.Where((_, i) => (i-x)%5==0).Count(v => v.Length == 0) == 5)
            return true;
    }
    return false;
}
...
var lines = System.IO.File.ReadAllLines("day04.txt").Where(line => line.Length > 0);
var drawOrder = lines.ElementAt(0).Split(',');
lines = lines.Skip(1);
...
// Read into memory. Now find winner
